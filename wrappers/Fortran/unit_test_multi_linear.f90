...
!** File Details **
!
!Filename: unit_test_multi_linear.f90
!Created: 05 September 2022
!Author:  W, Liu (derived from original 3D unit test by S. Kudo and unit test
!             single by S. M. Longshaw)
!Description: Unit test for Fortran wrapper using temporal_sampler_linear
!             to create and manage multiple MUI interfaces and associated sampler objects
...
program main
  ...
  type(c_ptr), target :: temporal_sampler_linear_1d=c_null_ptr
  type(c_ptr), target :: temporal_sampler_linear_2d=c_null_ptr
  type(c_ptr), target :: temporal_sampler_linear_3d=c_null_ptr
  real(c_double) :: tolerance=1e-37_c_double
  real(c_double) :: dt_neighbour=1.0_c_double
  ...
  call mui_create_temporal_sampler_linear_1d_f(temporal_sampler_linear_1d, dt_neighbour)

  !Fetch the value for tag "position" at location={fetch_point}={0} at fetch_time=0 for all MUI interfaces
  do i = 1, interface_count
     call mui_fetch_exact_linear_1d_f(uniface_pointers_1d(i)%ptr, "position"//c_null_char, fetch_point_1,  &
           fetch_time, spatial_sampler_exact_1d, temporal_sampler_linear_1d, fetch_result_1d)
     print *, "Fetched 1D interface value (linear temporal) = ",fetch_result_1d, " at the interface of ", interfaces1d(i),             &
           " domian of ", domain1d
  end do

  !Destroy created 1D MUI objects
  call mui_destroy_sampler_exact_1d_f(spatial_sampler_exact_1d)
  call mui_destroy_temporal_sampler_linear_1d_f(temporal_sampler_linear_1d)
  ...
  call mui_create_temporal_sampler_linear_2d_f(temporal_sampler_linear_2d, dt_neighbour)

  !Fetch the value for tag "position" at location={fetch_point}={0,0} at fetch_time=0 for all MUI interfaces
  do i = 1, interface_count
     call mui_fetch_exact_linear_2d_f(uniface_pointers_2d(i)%ptr, "position"//c_null_char, fetch_point_1, fetch_point_2, &
           fetch_time, spatial_sampler_exact_2d, temporal_sampler_linear_2d, fetch_result_2d)
     print *, "Fetched 2D interface value (linear temporal) = ",fetch_result_2d, " at the interface of ", interfaces2d(i),             &
           " domian of ", domain2d
  end do

  !Destroy created 2D MUI objects
  call mui_destroy_sampler_exact_2d_f(spatial_sampler_exact_2d)
  call mui_destroy_temporal_sampler_linear_2d_f(temporal_sampler_linear_2d)
  ...
  call mui_create_temporal_sampler_linear_3d_f(temporal_sampler_linear_3d, dt_neighbour)

  !Fetch the value for tag "position" at location={fetch_point}={0,0,0} at fetch_time=0 for all MUI interfaces
  do i = 1, interface_count
     call mui_fetch_exact_linear_3d_f(uniface_pointers_3d(i)%ptr, "position"//c_null_char, fetch_point_1, fetch_point_2, &
           fetch_point_3, fetch_time, spatial_sampler_exact_3d, temporal_sampler_linear_3d, fetch_result_3d)
     print *, "Fetched 3D interface value (linear temporal) = ",fetch_result_3d, " at the interface of ", interfaces3d(i),             &
           " domian of ", domain3d
  end do

  !Destroy created 3D MUI objects
  call mui_destroy_sampler_exact_3d_f(spatial_sampler_exact_3d)
  call mui_destroy_temporal_sampler_linear_3d_f(temporal_sampler_linear_3d)

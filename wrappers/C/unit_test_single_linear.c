...
/**
 * @file unit_test_single_linear.c
...
int main(int argc, char **argv) {
	...
	mui_temporal_sampler_linear_1d *temporal_sampler1d = mui_create_temporal_sampler_linear_1d(1.0);

	// Define point for fetch
	mui_point_1d fetch_point1d = { 0 };

	// Fetch the value for tag "position" at fetch_point at time=0
	double result1d = mui_fetch_exact_linear_1d(uniface1d, "position", fetch_point1d, 0, spatial_sampler1d,
			temporal_sampler1d);
	...
	mui_destroy_temporal_sampler_linear_1d(temporal_sampler1d);
	free(uri1d);
	...
	mui_temporal_sampler_linear_2d *temporal_sampler2d = mui_create_temporal_sampler_linear_2d(1.0);

	// Define point for fetch
	mui_point_2d fetch_point2d = { 0, 0 };

	// Fetch the value for tag "position" at fetch_point at time=0
	double result2d = mui_fetch_exact_linear_2d(uniface2d, "position", fetch_point2d, 0, spatial_sampler2d,
			temporal_sampler2d);
	...
	mui_destroy_temporal_sampler_linear_2d(temporal_sampler2d);
	free(uri2d);
	...
	mui_temporal_sampler_linear_3d *temporal_sampler3d = mui_create_temporal_sampler_linear_3d(1.0);

	// Define point for fetch
	mui_point_3d fetch_point3d = { 0, 0, 0 };

	// Fetch the value for tag "position" at fetch_point at time=0
	double result3d = mui_fetch_exact_linear_3d(uniface3d, "position", fetch_point3d, 0, spatial_sampler3d,
			temporal_sampler3d);
	...
	mui_destroy_temporal_sampler_linear_3d(temporal_sampler3d);
